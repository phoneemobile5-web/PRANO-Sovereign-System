...
export default function Dashboard() {
  ...
  const handleCreateProject = () => {
    const newProject = addProject({
      name: 'مهمة عالمية جديدة',
      description: 'مشروع يهدف لخدمة كوكب الأرض عبر الذكاء الاصطناعي',
      prompt: 'أنت مساعد ذكي مخصص لمشاريع التنمية المستدامة وحماية الكوكب.',
  ...
  if (!isLoaded) return (
    <div className="min-h-screen flex items-center justify-center bg-[#001a1a]">
      <div className="text-center space-y-4">
        ...
        <p className="text-sm font-black text-primary tracking-widest uppercase">جاري تشغيل مركز قيادة الكوكب...</p>
      </div>
    </div>
  );
    ...
    <div className="min-h-screen bg-background p-4 space-y-6 max-w-md mx-auto pb-24 font-body" dir="rtl">
      <header className="bg-card p-6 rounded-[2.5rem] border-2 border-primary/20 shadow-xl space-y-4 relative overflow-hidden">
        <div className="absolute top-0 right-0 p-8 opacity-5">
          <Globe className="w-32 h-32 rotate-12 text-primary" />
        </div>
        ...
        <div className="flex items-center justify-between relative z-10">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-3xl font-black text-foreground leading-none gold-gradient-text">منصة الأرض</h1>
              <p className="text-[11px] text-muted-foreground font-bold uppercase tracking-widest mt-1">مركز قيادة الاستدامة الكونية</p>
            </div>
          </div>
        ...
        <div className="grid gap-3 pt-2 relative z-10">
          ...
          <Button variant="outline" onClick={handleCreateProject} className="h-16 text-lg font-bold rounded-2xl w-full border-2 border-dashed border-primary/30 hover:bg-primary/5 transition-colors text-muted-foreground">
            <Plus className="ml-2 w-5 h-5" /> إنشاء مسودة مهمة جديدة
          </Button>
        </div>
      </header>
      ...
      <section className="space-y-4">
        <h2 className="text-xs font-black uppercase text-muted-foreground flex items-center gap-2 px-2 tracking-[0.2em]">
          <Globe className="w-4 h-4 text-primary" /> قائمة المهام النشطة ({projects.length})
        </h2>
        ...
        <div className="grid gap-4">
          {projects.length > 0 ? (
              ...
              <div key={project.id} className="relative group">
                      ...
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                          ...
                          <div className="flex flex-col items-start">
                            ...
                            <span className="text-[9px] text-muted-foreground font-bold uppercase tracking-wider">مهمة لخدمة الكوكب</span>
                          </div>
                        </div>
                ...
                <AlertDialog>
                  ...
                  <AlertDialogContent className="w-[90%] rounded-[2rem] border-4 bg-card" dir="rtl">
                    <AlertDialogHeader>
                      <AlertDialogTitle className="text-right text-2xl font-black text-destructive">إلغاء المهمة؟</AlertDialogTitle>
                      <AlertDialogDescription className="text-right font-medium text-lg leading-relaxed">
                        سيتم مسح بيانات مهمة "{project.name}" نهائياً من مركز القيادة.
                      </AlertDialogDescription>
                    </AlertDialogHeader>
            ...
            <div className="py-20 text-center border-4 border-dashed rounded-[3rem] opacity-30 bg-muted/5">
              ...
              <p className="text-lg font-black text-muted-foreground uppercase tracking-[0.2em]">بانتظار أول مهمة عالمية</p>
            </div>
          )}
        </div>
      </section>
      ...
      <nav className="fixed bottom-4 left-4 right-4 h-20 bg-card/90 backdrop-blur-2xl border-2 border-primary/20 flex items-center justify-around px-10 shadow-[0_10px_50px_rgba(0,0,0,0.5)] z-50 rounded-[2.5rem]">
        <Link href="/" className="flex flex-col items-center gap-1 text-primary scale-110">
          <Globe className="w-7 h-7 fill-current" />
          <span className="text-[10px] font-black uppercase">القيادة</span>
        </Link>
